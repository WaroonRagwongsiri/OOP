...
class Account(ABC):
	...
	@amount.setter
	def	amount(self, input_amount: float):
		if not isinstance(input_amount, float) and not isinstance(input_amount, int):
	...
	def _validate_channel_session(self, channel):
		...
		"""
		if not isinstance(channel, ATM_machine) and not isinstance(channel, EDC_machine):
			raise TypeError("Not ATM_machine or EDC_machine")
		if channel.is_using(self.__card) == False:
			raise PermissionError("Other Account is using this Machine")
		return True
	...
	def print_transactions(self):
		...
		"""
		print(f"\n--- History for {self.__acc_id} ({type(self).__name__}) Balance: {self.__balance:.2f} ---")
	...
	def _get_channel_id(self, channel):
		...
		if isinstance(channel, ATM_machine):
			channel_id = channel.atm_id
		if isinstance(channel, EDC_machine):
			channel_id = channel.edc_id
		if isinstance(channel, Counter):
			channel_id = channel.counter_id
	...
	def deposit(self, channel: 'Channel', amount: float):
		...
		self._create_transaction("D", channel.get_channel_type(), channel.get_channel_id(), amount, self.__balance)
	...
	def receive_transfer(self, amount, channel, source_acc_no):
		...
		"""
		if not isinstance(channel, Channel) or (not isinstance(amount, int) and not isinstance(amount, float) or not isinstance(source_acc_no, str)):
			raise TypeError("Invalid Type")
		self.__balance += amount
		self._create_transaction('TD', channel.get_channel_type(), channel.get_channel_id(), amount, self.__balance, source_acc_no)
...
class FixedAccount(Account):
	...
	def __init__(self, account_no, user, amount, term_months=12):
		...
		"""
		super().__init__(account_no, user, amount)
		self.__term_months: int = term_months
		self.__start_date: datetime = datetime.now()
		self.__maturity_date: datetime = self.__start_date + timedelta(days=term_months*30)
		self.__is_early_withdrawal: bool = False
...
class Card(ABC):
	...
	def validate_pin(self, pin_input):
		...
		"""
		if not isinstance(pin_input, str):
			raise TypeError("Invalid Type")
...
class Channel(ABC):
	...
	def add_bank(self, bank: 'Bank'):
		if not isinstance(bank, Bank):
			raise TypeError("Invalid Type for bank")
		self.__bank = bank
...
class ATM_machine(Channel):
	...
	def insert_card(self, card, pin):
		...
		"""
		if not isinstance(card, Card) or not isinstance(pin, str):
			raise TypeError("Invalid Type")
		if card.validate_pin(pin) == False:
			return False
...
class EDC_machine(Channel):
	...
	def is_using(self, card: DebitCard):
	...
	def swipe_card(self, card: DebitCard, pin: str):
		...
		- ถ้าไม่ถูก → return False
		"""
		if not isinstance(card, DebitCard) or not isinstance(pin, str):
			raise TypeError("Invalid Type")
		if card.validate_pin(pin) == False:
			return False
		self.__current_card = card
...
class Bank:
	...
	def __init__(self, name: str):
		...
		self.__atm_list: list[ATM_machine] = []
		self.__edc_list: list[EDC_machine] = []
		self.__counter_list: list[Counter] = []
	...
	def get_edc_by_id(self, edc_id):
		...
		if not isinstance(edc_id, str):
			raise TypeError("Invalid Type")
		for index, item in enumerate(self.__edc_list):
			edc: EDC_machine = item
			if edc.edc_id == edc_id:
				return edc
		return None
	...
	def search_account_from_card(self, card_no):
		...
		for index, item in enumerate(self.__user_list):
			user: User = item
			acc = user.search_account_from_card(card_no)
			if acc != None:
				return acc
		return None
...
class Transaction:
	...
	AVAILABLE_TYPE: list[str] = ["D", "W", "TW", "TD", "I", "P", "F"]
	...
	def __init__(self, type, channel_type, channel_id, amount, balance, target=None):
		...
		self.__channel_type : str = channel_type
		self.__channel_id : str = channel_id
		self.__amount : float = amount
		self.__balance : float = balance
		self.__target_acc_id : str = target
